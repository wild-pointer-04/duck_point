...
import numpy as np
import open3d as o3d
from sklearn.cluster import DBSCAN   # 需要 pip install scikit-learn
...
class DuckSegmenter:
    ...
    def get_clicks(self, rgb_image):
        ...
        window_name = "Duck Annotation - Frame 0"
...
def make_duck_pcd(nx6, voxel_size=0.005):
    """把鸭子点云转 open3d 格式，降采样，估算法向量"""
    if len(nx6) < 30:
        return None
...
def is_sane_transform(tf, max_trans_m=0.04, max_angle_deg=6.0):
    """
    校验变换矩阵是否合理：
    - 平移不超过 max_trans_m（相邻帧相机移动上限）
    - 旋转角度不超过 max_angle_deg
    鸭子离相机 ~0.5m，30fps，相机移速通常 < 3cm/frame
    """
    t = np.linalg.norm(tf[:3, 3])
    cos_a = np.clip((np.trace(tf[:3, :3]) - 1.0) / 2.0, -1.0, 1.0)
    angle_deg = np.degrees(np.arccos(cos_a))
    return t < max_trans_m and angle_deg < max_angle_deg, t, angle_deg


def duck_icp(src_pcd, dst_pcd, voxel_size, init_tf=None):
    """
    Duck-to-Duck Point-to-Plane ICP
    src/dst 均为鸭子点云（已算法向量）
    返回 (transform, fitness)
    """
    if init_tf is None:
        init_tf = np.eye(4)
    result = o3d.pipelines.registration.registration_icp(
...
def merge_duck_icp_v2(duck_arrays, voxel_size_duck=0.001,
                      max_trans_m=0.04, max_angle_deg=6.0):
    """
    修复版：Duck-to-Duck ICP 配准融合

    改进点：
    1. 用鸭子点云互配准（不是全场景），彻底避免桌面滑动问题
    2. 每帧校验变换幅度，超标直接拒绝（不再仅看 fitness）
    3. 每 10 帧选一个新关键帧（固定锚点，防止累积漂移）
    4. 拒绝帧计数超阈值时重置关键帧（检测相机大幅运动）
    """
    if not duck_arrays:
        return np.array([])

    n = len(duck_arrays)
    print(f"\n[ICP-v2] 开始 Duck-to-Duck 配准，共 {n} 帧...")

    icp_voxel = max(voxel_size_duck * 3, 0.003)  # ICP 用稍大体素，更稳

    # 初始化关键帧（第0帧）
    keyframe_pcd = make_duck_pcd(duck_arrays[0], icp_voxel)
    if keyframe_pcd is None:
        print("[ICP-v2] 第0帧鸭子点云无效")
        return np.array([])
    ...
    bad_count = 0
    keyframe_trans = np.eye(4)  # 关键帧到世界坐标的变换
    frames_since_keyframe = 0
    KEYFRAME_INTERVAL = 10       # 每 10 帧更新一次关键帧
    BAD_RESET_THRESH = 5         # 连续 5 帧失败则重置关键帧

    for i in range(1, n):
        src_pcd = make_duck_pcd(duck_arrays[i], icp_voxel)
        if src_pcd is None or len(src_pcd.points) < 20:
            bad_count += 1
            continue

        # Duck-to-Duck ICP（当前帧 → 关键帧）
        tf, fitness = duck_icp(src_pcd, keyframe_pcd, icp_voxel)

        # 变换幅度校验
        sane, t_dist, angle = is_sane_transform(tf, max_trans_m, max_angle_deg)

        if fitness < 0.35 or not sane:
            print(f"[ICP-v2] 帧{i}: fitness={fitness:.3f}, "
                  f"Δt={t_dist*100:.1f}cm, Δθ={angle:.1f}° → 拒绝")
            bad_count += 1

            # 连续失败太多，尝试重新用当前帧作为关键帧
            if bad_count >= BAD_RESET_THRESH:
                print(f"[ICP-v2] 帧{i}: 连续 {bad_count} 帧失败，重置关键帧")
                keyframe_pcd = src_pcd
                keyframe_trans = trans_global.copy()
                bad_count = 0
                frames_since_keyframe = 0
            continue
        ...
        # tf 是 src→keyframe 的变换，转换为世界坐标
        # world = keyframe_trans @ tf^-1 (因为 tf: src→keyframe)
        # 注意：ICP 输出是把 src 变换到 dst 的矩阵
        # 即 dst ≈ tf @ src，所以 src 在世界坐标 = keyframe_trans @ tf
        trans_curr = keyframe_trans @ tf
        ...
        curr_duck.transform(trans_curr)

        merged += curr_duck
        merged = merged.voxel_down_sample(voxel_size_duck)

        trans_global = trans_curr

        # 更新关键帧
        if frames_since_keyframe >= KEYFRAME_INTERVAL:
            keyframe_pcd = make_duck_pcd(duck_arrays[i], icp_voxel)
            if keyframe_pcd is not None:
                keyframe_trans = trans_curr.copy()
                frames_since_keyframe = 0
                print(f"[ICP-v2] 帧{i}: 更新关键帧")

        if i % 20 == 0:
            print(f"[ICP-v2] 帧{i}/{n}: fitness={fitness:.3f}, "
                  f"Δt={t_dist*100:.1f}cm, Δθ={angle:.1f}°, "
...
# DBSCAN 最终聚类过滤（Fix C）
# ══════════════════════════════════════════════════════
def keep_largest_cluster(pcd_nx6, eps=0.05, min_samples=10):
    """
    用 DBSCAN 找最大连通块（鸭子主体），丢弃孤立噪点和幽灵帧碎片
    eps=5cm 是鸭子尺度的合理邻域半径
    """
    if len(pcd_nx6) < min_samples * 2:
        return pcd_nx6
    pts = pcd_nx6[:, :3].astype(np.float32)
    ...
    kept = pcd_nx6[labels == largest]
    print(f"[DBSCAN] {len(pcd_nx6)} → {len(kept)} 点（最大 cluster={largest}，"
          f"共 {len(unique)} 个 cluster）")
    return kept
...
# ── 替换这一段 ──────────────────────────────────────
    # 原来: merged = merge_duck_icp(valid_scenes, valid_ducks, ...)
    # 改为:
    if len(valid_ducks) == 1:
        merged = valid_ducks[0]
        print("[INFO] 只有一帧，跳过ICP")
    else:
        merged = merge_duck_icp_v2(
            valid_ducks,
            voxel_size_duck=args.voxel_size,
            max_trans_m=0.05,    # 每帧最大平移 4cm（可根据采集速度调整）
            max_angle_deg=6.0)   # 每帧最大旋转 6°
    ...
    # ★ 新增：DBSCAN 最终聚类，保留鸭子主体，丢弃幽灵碎片
    merged = keep_largest_cluster(merged, eps=0.05, min_samples=10)
