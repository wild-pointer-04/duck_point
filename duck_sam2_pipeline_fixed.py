...
def merge_duck_icp(duck_arrays, voxel_size_duck=0.001,
                   max_trans_m=0.15,      # 放宽：手持绕行每帧可能移动10cm+
                   max_angle_deg=25.0,    # 放宽：手持绕行每帧可能转15-20°
                   keyframe_interval=5):  # 收紧：更频繁更新关键帧
    """
    Duck-to-Duck ICP 多帧融合（修复版）
 
    关键帧更新策略（修复核心）：
    ┌─ 每隔 keyframe_interval 帧，无条件用当前帧更新关键帧 ─┐
    │  不管这帧有没有被ICP接受                              │
    │  这样关键帧始终跟着相机走，不会越来越远               │
    └───────────────────────────────────────────────────────┘
 
    接受条件：
    ① fitness >= 0.35（更宽松，鸭子点云部分遮挡时fitness会偏低）
    ② 平移 < max_trans_m
    ③ 旋转角 < max_angle_deg
    """
    if not duck_arrays:
    ...
    # 关键帧在世界坐标系下的变换（初始=单位矩阵，即第0帧定义世界原点）
    ...
    for i in range(1, n):
        ...
        if fitness < 0.35:
            ...
            if not ok:
                ...
                # tf：src → kf；kf_world_tf：kf → world
        ...
        # ── 关键帧更新：按绝对帧号，无条件更新 ──
        # 这是修复漂移的关键：不管这帧有没有被接受，
        # 每隔 keyframe_interval 帧就把关键帧移到最近一个好帧上
        if (i - kf_idx) >= keyframe_interval:
            if accepted_this:
                ...
                if new_kf is not None:
                    ...
                    kf_idx = i
                    print(f"[ICP] 帧{i:3d}: ✓ 关键帧更新（正常）")
            else:
                # 这帧被拒绝了，但时间到了必须更新关键帧
                # 往前找最近一个点数足够的帧
                # 保守处理：关键帧位置不变，只重置帧计数器
                # 这样下一个好帧会从当前关键帧重新开始对齐
                kf_idx = i
                print(f"[ICP] 帧{i:3d}: ✗ 关键帧时间到但此帧被拒，"
                      f"保持旧关键帧位置，重置计数")
    ...
    if accepted / (n - 1) < 0.5:
        print(f"[WARN] 接受率低于50%！建议：")
        print(f"  --max_trans {max_trans_m + 0.05:.2f}  （当前{max_trans_m:.2f}，适当放宽）")
        print(f"  --max_angle {max_angle_deg + 5:.1f}   （当前{max_angle_deg:.1f}，适当放宽）")
        print(f"  --kf_interval {max(3, keyframe_interval-2)}   "
              f"（当前{keyframe_interval}，适当减小）")
...
def main():
    ...
    frames = reader.read_all(max_frames=50)
    if not frames:
        print("[ERROR] 没有配对到帧")
        sys.exit(1)
