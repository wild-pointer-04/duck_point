...
def select_static_frames(frames_rgb, motion_threshold=5.0, min_run=10):
    """
    筛选相机静止的帧。
    motion_threshold: 相邻帧平均像素差（灰度），超过此值认为在移动
    min_run: 最短连续静止段长度
    返回：静止帧的索引列表
    """
    import cv2
    grays = [cv2.cvtColor(f, cv2.COLOR_RGB2GRAY).astype(np.float32) for f in frames_rgb]
    
    diffs = [0.0]  # 第0帧没有前帧，默认静止
    for i in range(1, len(grays)):
        diff = np.mean(np.abs(grays[i] - grays[i-1]))
        diffs.append(diff)
    
    diffs = np.array(diffs)
    is_static = diffs < motion_threshold
    ...
    for i, s in enumerate(is_static):
        if s and run_start is None:
            ...
            if i - run_start >= min_run:
                static_indices.extend(range(run_start, i))
            run_start = None
    if run_start is not None and len(grays) - run_start >= min_run:
        static_indices.extend(range(run_start, len(grays)))
    
    print(f"[STATIC] 共 {len(frames_rgb)} 帧，静止帧: {len(static_indices)} 帧")
    print(f"[STATIC] 帧间差统计: min={diffs.min():.2f}, mean={diffs.mean():.2f}, max={diffs.max():.2f}")
    return static_indices, diffs
...
class DuckSegmenter:
    ...
    def segment_frame_sam1(self, rgb_image: np.ndarray, pos_points: np.ndarray, neg_points: np.ndarray):
        ...
        # 选得分最高的mask
        best_logit= masks[np.argmax(scores)]
        thresh = 10.0
        best = best_logit>thresh
        return best
...
class PointCloudFilter:
    ...
    def filter_by_mask_and_depth(
        pcd_nx6: np.ndarray,     # [X,Y,Z,R,G,B] 原始点云
        rgb_image: np.ndarray,   # HxW RGB图像
        mask: np.ndarray,        # HxW bool，True=鸭子
        color_camera_intrinsics,  # (fx,fy,cx,cy)
    ) -> np.ndarray:
# ... (前面的代码不变)
        ...
        # 【核心抢救：添加手动视差像素补偿】
        # 由于你的 RGB 镜头和 Depth 镜头有物理间距，导致投影后错位了大约几十个像素
        # 我们用一个魔法数字强行把 Mask 在 3D 空间中平移回来
        u_offset = 0  #  需要你微调的参数（往左偏或往右偏）
        v_offset = 0    # 通常相机是水平并排的，垂直方向误差很小

        # 投影到图像坐标（加上了补偿）
        u = (X / Z * fx + cx + u_offset).astype(np.int32)
        v = (Y / Z * fy + cy + v_offset).astype(np.int32)

        H, W = mask.shape
        
        # 保留我们上一轮加的空气墙截断（防止误伤更远的背景）
        min_z = Z[Z > 0.05].min() if len(Z[Z > 0.05]) > 0 else 0.1
        max_z_threshold = min_z + 0.15 
        
        in_bounds = (u >= 0) & (u < W) & (v >= 0) & (v < H) & (Z > 0) & (Z < max_z_threshold)
        # ... (后面的代码不变，继续保留我们加的 rgb_image 提取颜色的逻辑)
        ...
        # 【核心魔法：给点云上色！】
        # 从 RGB 图像中提取对应的像素颜色，并赋值给点云的 RGB 通道
        valid_u = u_valid[in_mask]
        valid_v = v_valid[in_mask]
        colors = rgb_image[valid_v, valid_u]
        filtered_pcd[:, 3:6] = colors
