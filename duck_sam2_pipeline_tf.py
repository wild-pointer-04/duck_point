...
def merge_duck_with_scene_icp(scene_arrays, duck_arrays, duck_voxel_size=0.001):
    """
    核心：使用全场景点云做 Colored ICP (带颜色的ICP) 帧间追踪。
    利用键盘、盒子的颜色特征打破平面的“打滑效应”，并加入防爆机制。
    """
    if not duck_arrays: return np.array([])
    
    print(f"\n[ICP] 开始全场景特征对齐(已启用颜色锁定 & 深度裁剪)，共 {len(scene_arrays)} 帧...")

    def make_clean_scene_pcd(nx6_array, voxel_size):
        # 【核心修复1】：裁剪深度。只取 1 米以内的点（桌面和物体），过滤掉远处的墙壁和地面噪声
    ...
    for i in range(1, len(scene_arrays)):
        ...
        # 【核心修复2】：严格使用 Colored ICP，按位置传参避免报错
        # Colored ICP 会同时计算几何误差和光度(颜色)误差，键盘的颜色会彻底锁死平移滑动
        result_icp = o3d.pipelines.registration.registration_colored_icp(
            source_scene, 
            target_scene, 
            0.03, # max_correspondence_distance (3厘米匹配容忍度)
            np.eye(4), # init (初始猜测)
            o3d.pipelines.registration.TransformationEstimationForColoredICP(),
            o3d.pipelines.registration.ICPConvergenceCriteria(relative_fitness=1e-6, relative_rmse=1e-6, max_iteration=50)
        )

        # 【核心修复3】：防爆机制！如果两帧之间匹配度极低(<15%)，说明跟丢了，拒绝应用这帧的错误矩阵
        if result_icp.fitness < 0.15:
            print(f"[WARN] 帧 {i} 匹配度过低 (fitness={result_icp.fitness:.2f})，触发防爆，跳过此帧。")
            continue
        ...
        if i % 10 == 0:
            print(f"[ICP] 全场景配准 {i}/{len(scene_arrays)} 帧完成，当前鸭子点数: {len(merged_duck_pcd.points)} (匹配度: {result_icp.fitness:.2f})")
...
# 帧间点对面 ICP (Source 匹配 Target)
        ...
        result_icp = o3d.pipelines.registration.registration_icp(
            source_scene, 
            target_scene, 
            max_correspondence_distance=0.05,  # <-- 修复了这里的参数名全称
            init=np.eye(4),
