...
def mask_to_pointcloud(depth: np.ndarray, color: np.ndarray,
    ...
    """
    【修复版】：先将深度点投影到彩色坐标系，再用 Mask 提取
    彻底解决 RGB 与 Depth 因为物理基线导致的错位问题。
    """
    H_d, W_d = depth.shape[:2]
    H_c, W_c = color.shape[:2]

    # 1. 把整张深度图变成深度相机坐标系下的 3D 点
    v_d, u_d = np.indices((H_d, W_d))
    v_d = v_d.flatten()
    u_d = u_d.flatten()
    z_d = depth.flatten()

    # 过滤无效深度（太近、太远、或者无数据为0的点）
    valid_d = (z_d > depth_min) & (z_d < depth_max)
    u_d, v_d, z_d = u_d[valid_d], v_d[valid_d], z_d[valid_d]
    ...
    # 2. 用外参 T_d2c 把点变换到彩色相机坐标系
    pts_d = np.stack([x_d, y_d, z_d, np.ones_like(z_d)], axis=1) # Nx4
    pts_c = (T_d2c @ pts_d.T).T # Nx4
    X_c, Y_c, Z_c = pts_c[:, 0], pts_c[:, 1], pts_c[:, 2]

    # 3. 把这些点重新投影到彩色图像的像素坐标 (u_c, v_c)
    cfx, cfy = c_intr["fx"], c_intr["fy"]
    ccx, ccy = c_intr["cx"], c_intr["cy"]
    
    # 防止除以 0 导致报错
    valid_z = Z_c > 0.001
    X_c, Y_c, Z_c = X_c[valid_z], Y_c[valid_z], Z_c[valid_z]

    u_proj = np.round((X_c / Z_c) * cfx + ccx).astype(int)
    v_proj = np.round((Y_c / Z_c) * cfy + ccy).astype(int)

    # 4. 过滤掉投射到画面外（越界）的点
    in_img = (u_proj >= 0) & (u_proj < W_c) & (v_proj >= 0) & (v_proj < H_c)
    u_proj, v_proj = u_proj[in_img], v_proj[in_img]
    X_c, Y_c, Z_c = X_c[in_img], Y_c[in_img], Z_c[in_img]

    # 5. 【核心步骤】只保留那些落在你画的 Mask 内部的点！
    in_mask = mask[v_proj, u_proj]
    X_c, Y_c, Z_c = X_c[in_mask], Y_c[in_mask], Z_c[in_mask]
    u_final, v_final = u_proj[in_mask], v_proj[in_mask]

    if len(X_c) == 0:
        return o3d.geometry.PointCloud()

    # 6. 获取颜色并生成 Open3D 点云
    colors_bgr = color[v_final, u_final].astype(np.float32) / 255.0
    colors_rgb = colors_bgr[:, ::-1]

    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(np.stack([X_c, Y_c, Z_c], axis=1))
