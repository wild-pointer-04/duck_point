# #!/usr/bin/env python3
# """
# 从 rosbag 提取所有帧目标物体点云并融合（最终修正版）

# 核心修正：
# 1. 外参=单位阵（点云已在 camera_color_optical_frame）
# 2. 颜色从彩色图投影采样（点云无 rgb 字段）
# 3. 自动检测并丢弃结构光近距离盲区帧
#    — 判据：mask 区域内有效点覆盖率 < min_coverage（默认 15%）
# """
...
#         """只取 xyz（点云无 rgb 字段），返回 (N, 3) float32"""
...
#     print("[WARN] 未找到 camera_info，使用默认值")
...
#                 if frame_idx % 10 == 0:
#                     print(f"  传播进度: {frame_idx+1}/{len(frames_rgb)}，mask像素={masks[frame_idx].sum()}")
...
#         cv2.namedWindow("点击目标 (左键=目标 右键=背景 Enter=确认 R=重置)", cv2.WINDOW_NORMAL)
#         cv2.setMouseCallback("点击目标 (左键=目标 右键=背景 Enter=确认 R=重置)", on_mouse)
#         while True:
#             img = disp.copy()
#             for p in pos:
...
#             cv2.imshow("点击目标 (左键=目标 右键=背景 Enter=确认 R=重置)", img)
...
# # 5. 点云过滤 + 盲区检测
# # ══════════════════════════════════════════
# def filter_points_by_mask(pcd_xyz, rgb_image, mask, fx, fy, cx, cy, W, H,
#                           min_coverage=0.10):
#     """
#     点云已在 camera_color_optical_frame，直接用内参投影。
    
#     min_coverage: mask 区域内点云覆盖率阈值
#                   覆盖率 = 有效3D点投影落在mask内的像素数 / mask总像素数
#                   低于此值说明该帧处于近距离盲区，返回 None

#     返回: (M, 6) float32 或 None（盲区帧）
#     """
#     N = len(pcd_xyz)
...
#     # ── 盲区检测 ──────────────────────────────────────────────
#     # 把所有落在图像内的点投影，统计落在 mask 内的比例
#     idx_inbounds = np.where(in_bounds)[0]
#     in_mask_flags = mask[v[idx_inbounds], u[idx_inbounds]]
#     pts_in_mask = in_mask_flags.sum()
#     mask_pixels = mask.sum()

#     coverage = pts_in_mask / max(mask_pixels, 1)

#     if coverage < min_coverage:
#         return None, coverage  # 盲区帧，丢弃

#     # ── 正常提取 ──────────────────────────────────────────────
#     in_mask = np.zeros(N, dtype=bool)
#     in_mask[idx_inbounds] = in_mask_flags

#     kept = np.where(in_mask)[0]
#     result = np.zeros((len(kept), 6), dtype=np.float32)
#     result[:, 0] = pcd_xyz[kept, 0]
#     result[:, 1] = pcd_xyz[kept, 1]
#     result[:, 2] = pcd_xyz[kept, 2]
...
#                         help="mask内点云覆盖率阈值，低于此值丢弃该帧（默认0.10=10%%）")
#     args = parser.parse_args()

#     tmp_dir = tempfile.mkdtemp(prefix="sam2_multi_")
...
#         # Step 3: SAM2
#         print("\n[Step 3] SAM2 视频分割（仅点击第一帧）...")
#         segmenter = VideoSegmenter(args.sam2_checkpoint)
#         masks = segmenter.segment_all_frames(frames_rgb, tmp_dir)

#         # Step 4: 逐帧提取
#         print(f"\n[Step 4] 逐帧提取目标点云（覆盖率阈值={args.min_coverage:.0%}）...")
#         print(f"{'帧':>4}  {'mask像素':>8}  {'提取点数':>8}  {'覆盖率':>7}  {'状态':>6}")
#         print("-" * 50)
...
#                 print(f"{i:>4}  {'—':>8}  {'—':>8}  {'—':>7}  跳过(mask空)")
#                 continue

#             result, coverage = filter_points_by_mask(
#                 pcd_raw, rgb, mask, fx, fy, cx, cy, W, H,
#                 min_coverage=args.min_coverage
#             )

#             if result is None:
#                 print(f"{i:>4}  {mask.sum():>8}  {'0':>8}  {coverage:>6.1%}  ⚠ 盲区")
#                 n_blind += 1
#             else:
#                 all_pts.append(result)
#                 print(f"{i:>4}  {mask.sum():>8}  {len(result):>8}  {coverage:>6.1%}  ✓")

#         print(f"\n有效帧: {len(all_pts)}，丢弃盲区帧: {n_blind}")

#         if not all_pts:
#             print("[ERROR] 无有效帧！尝试降低 --min_coverage 参数")
#             return

#         # Step 5: 融合
#         print("\n[Step 5] 融合...")
#         final_pcd = merge_and_denoise(all_pts, voxel_size=args.voxel_size)
...
class VideoSegmenter:
    ...
    def _interactive_click(rgb_image):
        ...
        # 使用纯英文的窗口名称
        win_name = "Interactive_Segmentation"
        ...
        while True:
            ...
            cv2.putText(img, "Left=Pos  Right=Neg  Enter=OK  R=Reset",
                        (10,30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,255), 2)
            cv2.imshow(win_name, img)  # 这里也要替换为 win_name
            key = cv2.waitKey(30) & 0xFF
